...
fn precompute_msm<V: VariableBaseMSM>(
    ...
    let total_sum = window_sums.iter().sum();
    Ok(total_sum)
}
    ...
    I: Iterator<Item = preprocess::Instance>,
{
    ...
    for instance in instances {
        ...
        for _ in 0..iterations {
            ...
            let _ = precompute_msm::<G>(&points[..], &scalars[..]).unwrap();
...
mod tests {
    ...
    fn test_msm_correctness_medium_sample() {
        ...
        match preprocess::FileInputIterator::open(&dir) {
            ...
            Err(_) => {
                preprocess::gen_vectors(8, 5, &dir);
            }
        }

        // computing the precomputation factors
        let size = 1 << 8;
        let c = if size < 32 {
            3
        } else {
