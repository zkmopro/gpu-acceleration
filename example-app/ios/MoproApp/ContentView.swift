...
struct ContentView: View {
    ...
    @State private var isMsmBenchmarkButtonEnabled = true
    ...
    var body: some View {
        VStack(spacing: 10) {
            ...
            Button("Run MSM Benchmark", action: runMsmBenchmarkAction).disabled(!isMsmBenchmarkButtonEnabled).accessibilityIdentifier("msmBenchmark")
...
extension ContentView {
    ...
    func runMsmBenchmarkAction() {
        textViewText += "Running MSM benchmark... "
        
        // Disable button during execution
        isMsmBenchmarkButtonEnabled = false
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            do {
                let inputSize: UInt32 = 1 << 16
                try! metalMsmBenchmark(inputSize: inputSize)
                
                let end = CFAbsoluteTimeGetCurrent()
                let timeTaken = end - start
                
                DispatchQueue.main.async {
                    self.textViewText += "\(String(format: "%.3f", timeTaken))s ✅\n"
                    self.isMsmBenchmarkButtonEnabled = true
                }
            } catch {
                DispatchQueue.main.async {
                    self.textViewText += "MSM benchmark failed: \(error.localizedDescription)\n"
                    self.isMsmBenchmarkButtonEnabled = true
                }
            }
        }
